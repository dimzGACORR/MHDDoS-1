    ...
    if result:
        ...
        if datetime.now() > expiration_date:
            ...
            vip_status = (
                ...
                f"📅 berakhir dalam: {expiration_date.strftime('%d/%m/%Y %H:%M:%S')}"
            )
    else:
...
def handle_addvip(message):
    ...
    if len(args) != 3:
        bot.reply_to(
            message,
            "❌ Formatnya tidak valid. Gunakan: `/vip <ID> <QUANTOS DIAS>`",
            parse_mode="Markdown",
        )
        return
...
def handle_ping(message):
    ...
    if not result:
        bot.reply_to(message, "❌ Hanya Admin Yang dapat menggunakan perintah ini, Buy bot pm @dimaspratama11.")
        return
